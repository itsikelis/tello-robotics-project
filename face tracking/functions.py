...
def findFace(img):
    ...
    if len(myFaceListArea) != 0:
        i = myFaceListArea.index(max(myFaceListArea))       # fill 
...
def trackFace(tello, info, w, pidYaw, pErrorYaw, pidFB, pErrorFB):
    ...
    area = info[1]
    area_range = [6200, 6800]   # minimum and maximum detected face areas (forward and backward movement)
    ...
    errorFB = area - (area_range[0] + area_range[1]) // 2            # deviation between measured frame area and mean reference freame area
    speedFB = pidFB[0] * errorFB + pidFB[2] * (errorFB - pErrorFB)   # PD Controller Implementation
    speedFB = int(np.clip(speedYaw, -40, 40))                        # forward/backwards speed saturation control

    if area < area_range[0] and area > area_range[1]:    # area outside limits -> move forward/backwards
